...
versionString = 'GeMS_ValidateDatabase.py, version of 11 July 2019'
...
# 11 July 2019  Made changes to use in arcmap
    ...
    for xx in fdsfc:
        ...
        if fds == 'CorrelationOfMapUnits':
            if not ('CMULines' in fcs):
                schemaErrors.append(
                    'Feature data set <span class="tables">' + fds + '</span>, feature class <span class="tables">CMULines</span> is missing')
            if not ('CMUMapUnitPolys' in fcs):
                schemaErrors.append(
                    'Feature data set <span class="tables">' + fds + '</span>, feature class <span class="tables">CMUMapUnitPolys</span> is missing')
            if not ('CMUText' in fcs):
                schemaErrors.append(
                    'Feature data set <span class="tables">' + fds + '</span>, feature class <span class="tables">CMUText</span> is missing')
...
def checkContent():
    ...
    for n in range(1, len(all_IDs)):
        if all_IDs[n - 1][0] == all_IDs[n][0]:
            duplicateIDs.append(
                space4 + '<span class="values">' + str(all_IDs[n][0]) + '</span>, tables <span class="tables">' + str(
                    all_IDs[n - 1][1]) + ' ' + str(all_IDs[n][1]) + '</span>')
    ...
    if loadTableValues('DataSources', 'DataSources_ID', dataSourcesIDs):
        ...
        for anID in allDataSourcesRefs:
            if anID[0] != None and anID[0] != lastID:
                ...
                if anID[0] not in dataSourcesIDs:
                    missingSourceIDs.append(space4 + '<span class="values">' + str(
                        anID[0]) + '</span>, cited in field <span class="fields">' + str(
                        anID[1]) + '</span> of table <span class="tables">' + str(anID[2]) + '</span>')
        ...
        missingSourceIDs.append(
            space4 + 'Error: did not find field <span class="fields">DataSources_ID</span> in table <span class="tables">DataSources</span>')
        unusedDataSources.append(
            space4 + 'Error: did not find field <span class="fields">DataSources_ID</span> in table <span class="tables">DataSources</span>')
    ...
    if not loadTableValues('DescriptionOfMapUnits', 'MapUnit', dmuMapUnits):
        missingDmuMapUnits.append(
            space4 + 'Error: did not find field <span class="fields">MapUnit</span> in table <span class="tables">DescriptionOfMapUnits</span>')
        unreferencedDmuMapUnits.append(
            space4 + 'Error: did not find field <span class="fields">MapUnit</span> in table <span class="tables">DescriptionOfMapUnits</span>')
    if not loadTableValues('StandardLithology', 'MapUnit', standardLithMapUnits):
        missingStandardLithMapUnits.append(
            space4 + 'Error: did not find field <span class="fields">MapUnit</span> in table <span class="tables">StandardLithology</span>')
        unreferencedStandardLithMapUnits.append(
            space4 + 'Error: did not find <span class="fields">field MapUnit</span> in table <span class="tables">StandardLithology</span>')
    ...
    for mu in allMapUnitRefs:
        if mu[0] != lastMU:
            ...
            if mu[0] not in dmuMapUnits:
                missingDmuMapUnits.append(
                    space4 + '<span class="values">' + str(mu[0]) + '</span>, cited in <span class="tables">' + str(
                        mu[1]) + '</span>')
            if mu[0] not in standardLithMapUnits:
                missingStandardLithMapUnits.append(
                    space4 + '<span class="values">' + str(mu[0]) + '</span>, cited in ' + str(mu[1]) + '</span>')
        ...
        for term in allGlossaryRefs:
            if str(term[0]) != 'None' and term[0] != lastTerm:
                ...
                if term[0] not in glossaryTerms:
                    ...
                    missingGlossaryTerms.append(
                        space4 + '<span class="values">' + thisTerm + '</span>, cited in field <span class="fields">' +
                        term[1] + '</span> of table <span class="tables"> ' + term[2] + '</span>')
...
def dmuRequiredValues():
    ...
    if numberOfRows(dmu) > 0:
        ...
        fields = ['OBJECTID', 'MapUnit', 'Name', 'FullName', 'Age', 'Description', 'GeoMaterial',
                ...
                for row in cursor:
                    if isNullValue(row[2]):
                        missingRequiredValues.append(
                            '<span class="tables">DescriptionOfMapUnits</span>, OBJECTID=' + str(row[0]) + ' Name')
                    if not isNullValue(row[1]):  # if MapUnit is set
                        for i in range(3, len(fields)):
                            if isNullValue(row[i]):
                                missingRequiredValues.append(
                                    '<span class="tables">DescriptionOfMapUnits</span>, OBJECTID=' + str(
                                        row[0]) + ', <span class="fields">' + fields[i] + '</span>')
        except:
...
def tableToHtml(html, table):
    ...
    for field in fields:
        ...
        'OBJECTID', 'Shape_Length', 'created_user', 'created_date', 'last_edited_user', 'last_edited_date'):
...
def writeContentErrors(outfl, errors, noErrorString, outFile, htmlFileCount):
    ...
    if len(errors) < 10:
        ...
        writefl.write(rdiv)
        # write link to outfl
        outfl.write(errors[0] + '<a href ="' + 'file://' + fName + '"><br>' + space4 + 'View (' + str(
            len(errors) - 1) + ') Errors. (Link to New File)</a><br><br>\n')
...
def writeOutput(outFile, tables, thisDatabase):
    ...
    outfl.write(
        '<h4> The items below have been found in the geodatabase, but are not required for a valid GeMS Database.</h4>')
    ...
    if 'StandardLithology' in tables:
        hFC = writeContentErrors(outfl, missingStandardLithMapUnits, 'No missing MapUnits in StandardLithology',
                                 outFile, hFC)
        hFC = writeContentErrors(outfl, unreferencedStandardLithMapUnits,
                                 'No unreferenced MapUnits in StandardLithology', outFile, hFC)
    ...
    if 'ExtendedAttributes' in tables:
        hFC = writeContentErrors(outfl, unreferencedIds,
                                 'No rows in ExtendedAttributes that reference nonexistent OwnerIDs or ValueLinkIDs',
                                 outFile, hFC)
        if 'GeologicEvents' in tables:
            hFC = writeContentErrors(outfl, unusedGeologicEvents,
                                     'No rows in GeologicEvents not referenced in ExtendedAttributes', outFile)
